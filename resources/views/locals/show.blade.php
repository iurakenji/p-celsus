...
@section('titulo')
Locais de Armazenamento - {{ $local->nome }}
@endsection
...
<div class="text-center">
    ...
    <form action=" {{ route('locals.update', ['local' => $local->id]) }} " method="post">
        @csrf
        <input type="hidden" name="_method" value="PUT">
            <div class="m-3">
                <label class="form-label" for='nome'>Nome: </label>
                <input class="form-control" placeholder="Nome"  type="text" id="nome" name="nome" value="{{ $local->nome }}">
            </div>
            <div class="m-3">
            <label class="form-label" for='descricao' title='Descrição'>Orientações: </label>
            <input class="form-control" type="text" id="descricao" name="descricao" value="{{ $local->orientacoes }}">
            </div>
...
<div class="d-grid gap-4 d-md-flex justify-content-md-center">
    ...
    <form action=" {{ route('locals.destroy', ['local' => $local->id]) }}" method="POST">
    @csrf
