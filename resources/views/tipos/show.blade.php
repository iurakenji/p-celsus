    ...
    <form action=" {{ route('tipos.update', ['tipo' => $tipo->id]) }} " method="post">
        ...
        <div class="row">
            <div class="input-field col s4">
                <input placeholder="Nome"  type="text" id="nome" name="nome" value="{{ $tipo->nome }}">
                <label for='nome'>Nome: </label>
            </div>
            <div class="input-field col s8">
            <label for='descricao' title='Descrição'>Descrição: </label>
            <input type="text" id="descricao" name="descricao" value="{{ $tipo->descricao }}">
            </div>
        </div><br><br>
</div>
