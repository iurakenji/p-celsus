...
@section('titulo')
Controle de Acesso - Ações - {{ $acao->nome }}
@endsection
...
<div class="row center" style="margin: 0px 20px ">
    ...
    <form action=" {{ route('acaos.update', ['acao' => $acao->id]) }} " method="post">
        @csrf
        <input type="hidden" name="_method" value="PUT">
        <div class="row">
            <div class="input-field col s4">
                <input placeholder="Nome"  type="text" id="nome" name="nome" value="{{ $acao->nome }}">
                <label for='nome'>Nome: </label>
            </div>
            <div class="input-field col s8">
            <label for='descricao' title='Descrição'>Descrição: </label>
            <input type="text" id="descricao" name="descricao" value="{{ $acao->descricao }}">
            </div>
        </div><br><br>
</div>
...
<form action=" {{ route('acaos.destroy', ['acao' => $acao->id]) }}" method="POST">
    @csrf
